...
1.  **Analisis Kode:** Perhatikan kode `PasswordReminder` yang terikat pada `MySQLConnection`.
    ...
    Jika method `connect`diubah menjadi `connect(String user, String pass)`, maka `PasswordReminder` akan error di baris `dbConnection.connect("jdbc:mysql://localhost/prod_db");`, karena jumlah parameter tidak sesuai dengan method baru.
    ...
    Jika return type `executeQuery` berubah `jadi String[]`, maka di `PasswordReminder`, tipe `List<String>` harus diganti menjadi String[] dan loop `for(String userEmail : usersToRemind)` masih bisa dipakai, tapi hitungan `.size()` harus diganti dengan `.length`.
...
2.  **Analisis Diagram:** Perhatikan **Class Diagram "Sebelum Refactoring"**.
    ...
    Baris `this.dbConnection = new MySQLConnection();` di constructor `PasswordReminder`, karena di situ `PasswordReminder` membuat sendiri objek MySQL, bukan menerima abstraksi.
    ...
    Karena setiap kali `PasswordReminder` dijalankan, ia membuat koneksi nyata ke database MySQL (tidak bisa diuji tanpa koneksi DB sungguhan).
...
3.  **Jelaskan Kegunaannya:** Menurut Anda, dalam situasi apa memiliki kelas "koneksi simulasi" seperti `SimulatedConnection` ini bisa sangat berguna selama proses pengembangan perangkat lunak?
    Kegunaan `SimulatedConnection` untuk unit testing atau development mode, agar bisa menjalankan logika bisnis tanpa tergantung database nyata.
...
1.  Meskipun `PasswordReminder` sekarang sudah tidak terikat pada kelas `MySQLConnection`, ia masih memiliki "pengetahuan" tentang `connection string` yang spesifik untuk MySQL. Menurut Anda, apakah ini masih bisa dianggap sebagai sebuah masalah atau "bau kode" (*code smell*) dalam desain kita? Jelaskan argumen Anda.
    Ya, ini masih *code smell*, karena `PasswordReminder` tahu detail spesifik tentang database.
2.  Tanpa perlu menulis kode, dapatkan Anda membayangkan sebuah cara atau strategi agar kelas `PasswordReminder` sama sekali tidak perlu tahu tentang detail `connectionString`? Jelaskan idenya secara konseptual.
    Solusinya connection string bisa diberikan lewat konstruktor atau konfigurasi eksternal (file config/env), bukan ditulis di dalam class.
...
2.  Perhatikan *constructor* baru pada kelas `PasswordReminder`. Apa keuntungan utama dari memberikan objek dependensi (seperti `MySQLConnection`) melalui *constructor* dibandingkan membiarkan `PasswordReminder` membuatnya sendiri?
    Keuntungan Constructor Injection, yaitu: Lebih fleksibel (bisa ganti dependensi kapan saja), mudah diuji (bisa menyuntikkan mock object), dan tidak terikat pada implementasi konkret.
3.  **Kaitkan dengan Class Diagram:** Perhatikan kembali **Class Diagram "Sesudah Refactoring"**. Jelaskan bagaimana hasil eksekusi kode di `main` method (Langkah 5) membuktikan bahwa `PasswordReminder` sekarang bergantung pada `DBConnectionInterface` dan bukan pada implementasi konkret (`MySQLConnection` atau `PostgreSQLConnection`). Fokus pada bagaimana panah dependensi pada diagram tercermin dalam fleksibilitas kode yang Anda amati.
    Kode di `Main` membuktikan bahwa `PasswordReminder` hanya mengenal `DBConnectionInterface`. Panah dependensi di diagram menunjukkan arah ke interface, bukan ke MySQL/PostgreSQL — terbukti dari fleksibilitas saat ganti koneksi di runtime tanpa ubah `PasswordReminder`.
